...
- **`WARN: no lastPolled in state` in the log** — `.notification-state.json` was missing, empty or unparseable, so that poll looked back only 1 hour (`$STATE_FALLBACK_HOURS`) instead of fetching every unread thread ever. Before this guard, an empty state file on 2026-09-23 replayed 50 months-old notifications at once. The next poll writes a fresh state; state is saved atomically (temp file + swap), so a half-written file shouldn't happen again. Anything older than the fallback window is simply not re-delivered.
